...
export default function Navbar() {
  ...
  // Welcome banner: show only once per session for 3 seconds
  const [showBanner, setShowBanner] = useState(() => !sessionStorage.getItem('tng_banner_shown'))
  ...
  // Auto-dismiss banner after 3 seconds, never show again this session
  useEffect(() => {
    if (!showBanner) return
    sessionStorage.setItem('tng_banner_shown', '1')
  ...
  const openDropdown = (label) => { clearTimeout(dropdownTimer.current); setDropdown(label) }
  const closeDropdown = () => { dropdownTimer.current = setTimeout(() => setDropdown(null), 150) }
  ...
  const bannerHeight = showBanner ? 'top-8' : 'top-0'

  return (
    <>
      {/* Welcome banner — shows ONCE per session for 3 seconds only */}
      <div className={`fixed top-0 left-0 right-0 z-50 bg-gold-500 overflow-hidden transition-all duration-500 ${showBanner ? 'h-8' : 'h-0'}`}>
        <div className="max-w-7xl mx-auto px-6 h-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 ml-auto">
            <a href="tel:+919082690060" className="flex items-center gap-1 font-body text-charcoal-950 text-[10px] tracking-wider hover:text-charcoal-700 transition-colors">
              <Phone size={9} /> +91 90826 90060
            </a>
            <button onClick={() => setShowBanner(false)} className="text-charcoal-800 hover:text-charcoal-950 text-xs leading-none">✕</button>
          </div>
        </div>
      </div>

      <header className={`fixed left-0 right-0 z-50 transition-all duration-300 ${navBg} ${bannerHeight}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-20">
            {/* Logo */}
            <Link to="/" className="flex items-center gap-3 group shrink-0">
              <img src="/tng-logo.png" alt="TNG Hotels & Banquets"
              ...
              {navLinks.map((link) =>
                link.children ? (
                  <div key={link.label} className="relative"
                    ...
                    <button className="font-body font-medium tracking-widest uppercase text-xs text-white hover:text-gold-400 transition-colors flex items-center gap-1">
                      {link.label}
                  ...
                  <Link key={link.path} to={link.path}
                    className={`font-body font-medium tracking-widest uppercase text-xs transition-colors relative group
            ...
            <div className="hidden lg:flex items-center gap-4">
              {user ? (
                ...
                <Link to="/login" className="font-body font-medium tracking-widest uppercase text-xs text-white hover:text-gold-400 transition-colors">
                  Sign In
                </Link>
              )}
              <Link to="/rooms" className="bg-gold-500 hover:bg-gold-600 text-white font-body font-medium tracking-widest uppercase text-[10px] px-6 py-2.5 transition-all duration-300 border border-gold-500 hover:border-gold-600 hover:shadow-lg hover:shadow-gold-500/30 active:scale-95">
                Book Now
              </Link>
            </div>

            {/* Mobile toggle */}
            <button className="lg:hidden p-1.5 text-white hover:text-gold-400 transition-colors" onClick={() => setOpen(!open)}>
