  ...
  const submitForgot = async e => {
    ...
    try {
      ...
      if (status === 404) {
        ...
        // Even if API not implemented, show success for UX
        setForgotSent(true)
      }
    } finally { setLoading(false) }
  }
          ...
          {tab === 'login' ? (
            <>
              ...
              <form onSubmit={submit} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      name="password" type={show ? 'text' : 'password'} required value={form.password} onChange={handle}
                      className="input-luxury pr-12" placeholder="••••••••"
                      autoComplete="current-password"
                    />
                    <button type="button" onClick={() => setShow(!show)}
                      className="absolute right-4 top-1/2 -translate-y-1/2 text-charcoal-400 hover:text-gold-500 transition-colors">
                ...
                <button type="submit" disabled={loading}
                  className="btn-gold w-full justify-center py-3.5 disabled:opacity-60 text-sm">
                  {loading
              ...
              <div className="mt-6 pt-6 border-t border-charcoal-100 text-center">
                <p className="font-body text-sm text-charcoal-400">
                  New to TNG?{' '}
                  <Link to="/register" className="text-gold-600 hover:text-gold-700 font-semibold transition-colors">
            ...
            <>
              <button onClick={() => { setTab('login'); setError(''); setForgotSent(false) }}
                className="flex items-center gap-2 text-charcoal-400 hover:text-gold-500 font-body text-xs tracking-widest uppercase mb-8 transition-colors">
                ← Back to Sign In
              </button>

              <div className="mb-8">
                <h1 className="font-display text-charcoal-950 text-4xl font-light">Reset Password</h1>
                <p className="font-body text-charcoal-400 text-sm mt-2">Enter your email and we'll send reset instructions</p>
              </div>
              ...
              {forgotSent ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="font-display text-2xl text-charcoal-900 font-light mb-2">Email Sent!</h3>
                  <p className="font-body text-charcoal-400 text-sm mb-6">
                    If an account exists for <strong>{forgotEmail}</strong>, you'll receive password reset instructions shortly.
                  </p>
                  <p className="font-body text-charcoal-300 text-xs mb-6">
                    Don't see it? Check your spam folder or contact us at<br />
