...
// Forgot password — sends instruction (email integration optional)
router.post('/forgot-password',
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!result.rows.length) {
        // Don't reveal if email exists for security, but for demo we do
        return res.status(404).json({ error: 'No account found with this email' });
      }
      // In production: send actual reset email here
      // For demo: just return success
      res.json({
        message: 'Password reset instructions sent. Please contact reservations@tnghotels.com or call +91 98765 43210 for immediate assistance.',
